...
public class RawgProvider implements GameProvider {
    ...
    @Override
    public List<ListGameRawgDto> fetchAllGames(Long pageId) {
        // faire la function fetch en se basant sur le DTO et l'api rawg
        URI urlCleaner = UriComponentsBuilder.fromUriString(
            config.getRawgUrl() + "/games"
        )
        ...
        RawgResponse response = restClient
            .get()
            .uri(urlCleaner)
            .retrieve()
    ...
    public String gameDetail(Long id) {
        String urlCleaner = UriComponentsBuilder.fromUriString(
            config.getRawgUrl() + "/games/{id}"
        )
            .queryParam("key", config.getRawgApiKey())
            .encode()
            .build(id)
        ...
        return restClient
            ...
            .onStatus(HttpStatusCode::is4xxClientError, (req, res) -> {
                ...
                throw new RestClientException(
                    "Erreur inattendue Rawg : " + res.getStatusCode()
                );
            })
            .body(String.class);
    }
    ...
    public void debug() {
        ...
        try {
            ...
            System.out.println(
          //      "la taille de la liste : " + fetchAllGames(1L).size()
            );
           //
           //  System.out.println("Test API RAWG : " + fetchAllGames(1L));
        } catch (Exception e) {
            System.err.println(
                "Erreur lors du debug @PostConstruct : " + e.getMessage()
            );
            /*System.err.println(
                "Erreur lors du debug @PostConstruct avec l'uri : " + uriTest()
            );*/
        }
    }
}
